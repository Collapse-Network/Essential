 ...
 * Homepage: https://github.com/BakuTeam/Essential
 */
...
class Server{
	...
	public function __construct(
		...
		try{
			...
			$this->logger->info("Starting Server for Minecraft Bedrock Edition " . ProtocolInfo::MINECRAFT_VERSION_MIN . " - " . ProtocolInfo::MINECRAFT_VERSION_NETWORK);
			...
			$this->logger->info("Server started(" . strval(round(microtime(true) - $this->startTime, 3)) . "s)");
	...
	private function startupPrepareConnectableNetworkInterfaces(
		...
		if($rakLibRegistered){
			$this->logger->info("Starting server on " . $prettyIp . ":" . $port);
		}
		if($useQuery){
			if(!$rakLibRegistered){
