...
final class BlockStateToObjectDeserializer implements BlockStateDeserializer{
	...
	private function registerSimpleDeserializers() : void{
		...
		$this->mapSimple(Ids::OBSERVER, fn() => Blocks::OBSERVER());
	...
	private function registerDeserializers() : void{
		...
		$this->map(Ids::IRON_CHAIN, function(Reader $in) : Block{
			return Blocks::CHAIN()
				->setAxis($in->readPillarAxis());
		});
		...
		$this->map(Ids::GRINDSTONE, function(Reader $in) : Block{
			$in->ignored(StateNames::ATTACHMENT);
			$in->ignored(StateNames::DIRECTION);
		...
		$this->map(Ids::SCAFFOLDING, function(Reader $in) : Block{
			$in->ignored(StateNames::STABILITY);
			$in->ignored(StateNames::STABILITY_CHECK);
		...
		$this->map(Ids::LIGHTNING_ROD, function(Reader $in) : Block{
			$in->ignored(StateNames::POWERED_BIT); //TODO
