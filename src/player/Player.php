...
class Player extends Human implements CommandSender, ChunkListener, IPlayer{
	...
	protected function setPosition(Vector3 $pos) : bool{
		...
		$targetWorld = $pos instanceof Position && $pos->isValid() ? $pos->getWorld() : $oldWorld;
		if($oldWorld !== null && $targetWorld !== null && $targetWorld !== $oldWorld && $targetWorld->isLoaded()){
			foreach($this->usedChunks as $index => $status){
				World::getXZ($index, $X, $Z);
				$this->getNetworkSession()->stopUsingChunk($X, $Z, true);
			}
		}
	...
	public function syncBlocks(array $blocks) : void {
		...
		foreach ($this->getWorld()->createBlockUpdatePackets($this?->getNetworkSession()->getTypeConverter(), $blocks) as $packet) {
	...
	public function attackEntity(Entity $entity, ?Vector3 $clientClickPos = null, ?Vector3 $clientPlayerPos = null) : bool{
		...
		if($ev->getFinalDamage() > 0){
			//Include all synchronous hit effects in an early packet batch.
			$this->networkSession?->flushLatencySensitiveOutput();
			if($entity instanceof Player && $entity !== $this){
				$entity->networkSession?->flushLatencySensitiveOutput(true);
			}
		}
